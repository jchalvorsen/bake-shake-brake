...
z_dir = min(pts_car(:,3));
pts_car = [pts_car(:,1) - m(1), pts_car(:,2) - m(2), pts_car(:,3) - z_dir];
...
for t = 1:max(size(ts));
    ...
    %t = 8;
    pts_car = [pts_car(:,1:2), pts_car(:,3) + ts(t)];
    
    % combining car and bridge mappings:
    mapping = zeros(length(pts_car),1);
    for i = 1:length(pts_car)
        for j = 1:length(pts)
            if sum(pts_car(i,:) == pts(j,:)) == 3
                mapping(i) = j;
            end
        end
    end
    ...
    newpoints = [pts; pts_car];
    newelements = [el; el_car2];
    %[newpoints el] = getLargestConnectedDomain(newpoints, el);
    ...
    density = @(x) 7750 + (x(3) > 8)*50000; % kg/ m^3
